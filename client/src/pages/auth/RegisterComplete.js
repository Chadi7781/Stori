...
//         <span  className="text-sm text-gray-600 hover:underline-none">Create Account</span></Link>
...
function RegisterComplete({history}) {
  ...
  const [formValues,setFormValues]=useState(initialValues);
    ...
    useEffect(() => {
      ...
      setFormValues({email: window.localStorage.getItem('emailForRegistration') });
 ...
 const handleSubmit = async (e) => {
    ...
    try {
      const result = await signInWithEmailLink (auth,formValues.email,window.location.href)
  ...
  <div className="flex items-center justify-center bg-gray-100 p-12 sm:mx-auto ">
     ...
     <div className="w-full sm:max-w-md p-5 mx-auto">
      <h2 className="mb-8 text-center text-5xl font-extrabold">Register Complete </h2>
...
<div className="mt-4">
        <label htmlFor="password" className="block mb-1" for="password">Password</label>
        <input 
        onChange={handleChange}
        name="password"
    ...
    transition duration-500 ease-in-out transform border-transparent rounded-lg bg-gray-200 
     focus:border-blueGray-500 focus:bg-white dark:focus:bg-gray-800 focus:outline-none focus:shadow-outline
      focus:ring-2 ring-offset-current ring-offset-2 ring-gray-400 
        `}/>      </div> */}
      <div className="mt-6">
      <ButtonPrimary text="Complete Registration"  className="ml-8  w-full justify-center flex items-center px-6 py-3"/>
